...
resource "docker_container" "topaz-ahbroker" {
  depends_on = [
    null_resource.topaz-build,
    docker_container.topaz-db,
    null_resource.topaz-ahbroker,
  ]
...
resource "docker_container" "topaz-connect" {
  depends_on = [
    null_resource.topaz-build,
    docker_container.topaz-build,
    null_resource.topaz-connect,
  ]
...
resource "docker_container" "topaz-game" {
  depends_on = [
    null_resource.topaz-build,
    docker_container.topaz-connect,
    null_resource.topaz-game,
  ]
  image    = docker_image.topaz-game.latest
  count    = length(keys(local.single))
  name     = local.single[element(keys(local.single), count.index)]["name"]
  hostname = local.single[element(keys(local.single), count.index)]["name"]
  ...
  ports {
    protocol = "udp"
    internal = local.single[element(keys(local.single), count.index)]["zoneport"]
    external = local.single[element(keys(local.single), count.index)]["zoneport"]
  }
  env = [
    "MYSQL_HOST=${docker_container.topaz-db.name}",
    "MYSQL_LOGIN=${var.mysql_login}",
    "MYSQL_DATABASE=${var.mysql_database}",
    "MYSQL_PASSWORD=${var.mysql_password}",
    "zoneids=${local.single[element(keys(local.single), count.index)]["zoneids"]}",
    "zoneport=${local.single[element(keys(local.single), count.index)]["zoneport"]}",
  ]
  restart      = "always"
  network_mode = docker_network.topaz_network.name
}
